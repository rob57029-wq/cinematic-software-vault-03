...
const SoftwareCard: React.FC<SoftwareCardProps> = ({ software, onClick, className }) => {
  ...
  const getSoftwareIcon = (): LucideIcon => {
    ...
    // Game-specific icons
    if (name.includes('gamepad2')) return Gamepad2;
    ...
    if (name.includes('mountain')) return Mountain;
    ...
    if (name.includes('radiation')) return Radiation;
    if (name.includes('cloud')) return Cloud;
    if (name.includes('users')) return Users;
    if (name.includes('crosshair')) return Crosshair;
    ...
    if (name.includes('unlock')) return Unlock;
