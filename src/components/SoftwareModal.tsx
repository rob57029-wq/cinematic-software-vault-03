...
import { 
  ...
  Settings, BookOpen, Zap, Database, Archive, Gamepad2, Circle,
  Leaf, Fish, Sword, Train, Sparkles, Rocket, Car, Mountain,
  Target, Crown, ShieldCheck, Globe, Radiation, Cloud, Users,
  Crosshair, Heart, Wrench, Egg, Trophy, Circle as Football, Unlock
} from 'lucide-react';
...
const SoftwareModal: React.FC<SoftwareModalProps> = ({ software, isOpen, onClose }) => {
  ...
  const getSoftwareIcon = (softwareName: string): LucideIcon => {
    ...
    // Game-specific icons based on icon field
    if (softwareName === 'game' || name.includes('game')) return Gamepad2;
    if (softwareName === 'code' || name.includes('executor') || name.includes('script')) return Code;
    if (softwareName === 'settings' || name.includes('mod menu')) return Settings;
    if (softwareName === 'gamepad2' || name.includes('forza') || name.includes('rdr')) return Gamepad2;
    if (softwareName === 'box' || name.includes('minecraft')) return Box;
    if (softwareName === 'edit' || name.includes('editor')) return Edit;
    if (softwareName === 'unlock' || name.includes('unlock')) return Unlock;
    if (softwareName === 'circle' || name.includes('pool')) return Circle;
    if (softwareName === 'palette' || name.includes('skin changer')) return Palette;
    if (softwareName === 'leaf') return Leaf;
    if (softwareName === 'zap') return Zap;
    if (softwareName === 'fish') return Fish;
    if (softwareName === 'sword') return Sword;
    if (softwareName === 'train') return Train;
    if (softwareName === 'sparkles') return Sparkles;
    if (softwareName === 'rocket') return Rocket;
    if (softwareName === 'car') return Car;
    if (softwareName === 'mountain') return Mountain;
    if (softwareName === 'target') return Target;
    if (softwareName === 'crown') return Crown;
    if (softwareName === 'shield-check') return ShieldCheck;
    if (softwareName === 'globe') return Globe;
    if (softwareName === 'radiation') return Radiation;
    if (softwareName === 'cloud') return Cloud;
    if (softwareName === 'users') return Users;
    if (softwareName === 'crosshair') return Crosshair;
    if (softwareName === 'heart') return Heart;
    if (softwareName === 'wrench') return Wrench;
    if (softwareName === 'egg') return Egg;
    if (softwareName === 'trophy') return Trophy;
    if (softwareName === 'football' || name.includes('fifa') || name.includes('fc ')) return Football;
    if (softwareName === 'eye') return Database;
    if (softwareName === 'radar') return Archive;
    if (softwareName === 'menu') return Settings;
  ...
  const IconComponent = getSoftwareIcon(software.icon || software.name);
